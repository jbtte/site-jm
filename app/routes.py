...
@login_required
def index():
    balance = Balance.current_balance(1)
    account_statement = Balance.account_statement(1)
    form = ParentInput()

    if form.validate_on_submit():
        op = Balance(ammount=form.ammount.data, operation=int(form.operation.data), description=form.description.data, timestamp=form.timestamp.data , user_id=1)
        db.session.add(op)
        db.session.commit()
